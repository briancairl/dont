...
template <typename F, typename S> struct IfElse<true, F, S> { using type = F; };
...
template <typename T> constexpr T &&forward(remove_reference_t<T> &v) {
...
struct PackSize<FirstPackT, SecondPackT, OtherPackTs...>
    : PackSize<FirstPackT> {
  static_assert(PackSize<FirstPackT>() ==
                    PackSize<SecondPackT, OtherPackTs...>(),
                "pack sizes do not match");
};

template <typename... PackLikeTs>
constexpr auto pack_size_v = PackSize<PackLikeTs...>::value;
