...
struct IsMember<QueryT, Pack<Ts...>>
    : Constant<bool, (is_same_v<QueryT, Ts> or ...)> {};

template <template <typename, typename> class Condition> struct Invert {
  template <typename QueryT, typename PackLikeT>
  struct eval : Constant<bool, !Condition<QueryT, PackLikeT>::value> {};
};
